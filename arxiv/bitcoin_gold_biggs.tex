...
\begin{mdframed}[linewidth=1.5pt,linecolor=black!60,backgroundcolor=gray!5,
  innertopmargin=4pt,innerbottommargin=4pt,skipabove=6pt,skipbelow=6pt]
...
\begin{itemize}[leftmargin=*]
    ...
    \end{itemize}
    \item \textbf{Patience required:} Boom-bust cycles last roughly 40 months. The model predicts the trend, not any particular month. An investor who buys at a peak and sells at a trough can lose money even when the model is right. These returns assume a horizon of at least 3--5 years and the ability to wait out half-cycle busts of about 20 months before selling.
\end{itemize}
\end{mdframed}
...
The model was fit on data through December 2024. It knew nothing about what came next. Figure~\ref{fig:projection} shows what came next: thirteen months of Bitcoin's Gold value, through January 27, 2026, plotted as red squares. Every one of them falls close to the orange trend line, well within $\pm$1$\sigma$. That is tighter than expected from chance alone, and it suggests the model is tracking something real rather than memorizing noise. (The unusually tight scatter may signal falling volatility, a possibility tested in Section 7.)
...
\begin{itemize}
    \item \textbf{Gompertz:} $\ln(R) = C + gt + A\exp(-\exp(-kt))$
    \item \textbf{Logistic-in-log:} $\ln(R) = C + gt + A/(1 + \exp(-kt))$
\end{itemize}

\begin{table}[H]
\centering
\caption{Model class sensitivity: training fit vs.\ held-out prediction}
\label{tab:sensitivity}
\begin{tabular}{@{}lccccc@{}}
\toprule
\textbf{Model} & \textbf{Params} & \textbf{$R^2$ (train)} & \textbf{RMSE (train)} & \textbf{RMSE (test)} & \textbf{Test within $1\sigma$} \\
\midrule
...
Look at the training columns first. All three saturating forms are nearly identical: $R^2 \approx 0.91$, RMSE around 0.47. Training data alone cannot distinguish them. Now look at the test columns. The saturating exponential has the lowest test RMSE (0.222) and is the only model that puts all 13 held-out points within $1\sigma$. The Gompertz and logistic forms, which saturate more abruptly, systematically underpredict during the boom phase (Figure~\ref{fig:projection}, faint lines). Giving them a fourth free parameter makes things worse; their test RMSE rises slightly, the signature of overfitting. So the choice of functional form is not cosmetic. Among models of equal or greater complexity, the one built on the supply-differential asymptote is the one that generalizes. Thirteen months of test data is short compared to a full boom-bust cycle of roughly 40 months, so Section 10 introduces a scorecard that accumulates evidence month by month over years.

\subsection{Parameter Sensitivity}

The model has three fitted parameters. How much do five-year and ten-year projections change if each one shifts by one standard error? Table~\ref{tab:param_sensitivity} answers this by perturbing each parameter individually while holding the others fixed.

\begin{table}[H]
\centering
\caption{Parameter sensitivity: effect of $\pm$1 SE perturbations on projections}
\label{tab:param_sensitivity}
\begin{tabular}{@{}lccccc@{}}
\toprule
\textbf{Scenario} & \textbf{Fitted value} & \textbf{SE} & \textbf{2030 (oz)} & \textbf{2035 (oz)} & \textbf{$\Delta$ 2030} \\
\midrule
Baseline & --- & --- & 36.1 & 42.4 & --- \\
\midrule
$C$ $+$1 SE & $-2.252$ & $0.171$ & 42.8 & 50.3 & $+$19\% \\
$C$ $-$1 SE & & & 30.4 & 35.7 & $-$16\% \\
\midrule
$A$ $+$1 SE & $\phantom{-}5.618$ & $0.177$ & 42.9 & 50.5 & $+$19\% \\
$A$ $-$1 SE & & & 30.3 & 35.5 & $-$16\% \\
\midrule
$\lambda$ $+$1 SE & $\phantom{-}0.283$ & $0.028$ & 37.1 & 42.7 & $+$3\% \\
$\lambda$ $-$1 SE & & & 34.6 & 41.7 & $-$4\% \\
\bottomrule
\end{tabular}
\end{table}

The baseline ($C$) and adoption amplitude ($A$) each shift the projections by roughly $\pm$19\%, while the decay rate ($\lambda$) moves them by only $\pm$3\%. This makes physical sense: $C$ and $A$ set the overall level of the curve, so any error in either one scales the entire projection up or down. The decay rate, by contrast, governs how quickly adoption fades; by 2030, fifteen years after the start of the data, adoption has already largely decayed regardless of whether $\lambda$ is 0.255 or 0.311. The projections are therefore robust to uncertainty in the speed of adoption decay, but sensitive to the level parameters---exactly the uncertainty already captured by the $\pm$1$\sigma$ and $\pm$2$\sigma$ confidence bands in Section~6.

\subsection{Boom-Bust Cyclicality}

Subtracting the long-term trend, the residuals tell a story of their own. Look at Figure~\ref{fig:projection}: the price rides above the orange line for a stretch, then drops below it. The most recent boom began in early 2024 and turned to bust around October 2025.

How regular is this cycle? Figure~\ref{fig:autocorrelation} answers the question directly. The autocorrelation function of the training residuals peaks at lag 40 ($r = +0.67$): a boom today predicts another boom 40 months later, and likewise for busts. The cycle repeats roughly every 40 months, about three and a third years from peak to peak. Overlaying each residual with its value 40 months earlier confirms the pattern: of 133 months, 91 show same-sign residuals at the 40-month lag.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{figure_autocorrelation.png}
    \caption{Boom-bust cyclicality in model residuals. Top: log-space residuals (actual minus predicted) show alternating positive and negative runs. Middle: autocorrelation function of training residuals with 95\% confidence bands; the positive peak at lag 40 ($r = +0.67$) identifies a $\sim$40-month cycle. Bottom: residuals overlaid with a 40-month circular lag confirm the cycle: booms align with booms and busts with busts. Green shading indicates months where the current residual and the residual 40 months earlier share the same sign. 91 of 133 months show this alignment.}
    \label{fig:autocorrelation}
\end{figure}

This cyclicality matters for everything that follows. The model predicts a trend, not a price on any given Tuesday. An investor who buys at the top of a boom and is forced to sell at the bottom of the next bust can lose money even if the model is perfectly correct. The projections in this paper (Sections 6 and 9) assume a horizon long enough to ride out at least one full cycle: three to five years at minimum, with the ability to sit through half-cycle busts of about 20 months. Anyone who cannot wait that long is not investing on the trend. They are betting on which part of the cycle they happen to catch.
...
The Bitcoin--Gold comparison has attracted attention from institutions and independent researchers alike. Deutsche Bank~\cite{deutschebank2025}, Fidelity~\cite{fidelity2024}, WisdomTree~\cite{wisdomtree2025}, and NYDIG~\cite{cipolaro2025} have published analyses recognizing the parallel, and early academic work established that a long-term relationship exists between the two assets~\cite{zwick2019}. Dyhrberg~\cite{dyhrberg2016} showed via GARCH analysis that Bitcoin exhibits volatility clustering similar to Gold's, a finding this paper's documentation of declining volatility extends.

A rich tradition of power-law modeling for Bitcoin in USD terms begins with Santostasi~\cite{santostasi2024}, who fits $P \propto t^{5.8}$ to BTC/USD with $R^2 \approx 0.95$. Burger~\cite{burger2019} extended this into a ``power-law corridor'' with upper and lower bands that capture boom-bust regimes. These models describe BTC/USD, not the BTC/Gold ratio, but the ideas transfer directly. The live dashboard at B1M.io~\cite{b1m2026} applies a power law ($\beta = 5.35$, $R^2 = 0.952$) specifically to Bitcoin priced in gold ounces, with projections through 2045.

The most directly relevant prior work is by Perrenod~\cite{perrenod2021,perrenod2024a,perrenod2025}, who has published extensively on Bitcoin priced in gold. His power-law fit yields $k = 5.47$ for BTC/Gold ($R^2 = 0.94$)~\cite{perrenod2024a}. More importantly, he fits a Weibull S-curve ($f = 1 - \exp[-(t/c)^k]$) to model Bitcoin \emph{displacing} Gold---a saturating functional form conceptually identical to the adoption term $A(1 - e^{-\lambda t})$ used here~\cite{perrenod2021}. He also identifies two volatility regimes (``laminar'' and ``turbulent'') using a Gaussian mixture model and Hidden Markov Model, with the turbulent regime showing $\approx$3$\times$ higher volatility and a roughly four-year cycle period~\cite{perrenod2025}. In a direct comparison, Perrenod~\cite{perrenod2024b} finds that the Weibull S-curve and power law achieve similar $R^2$ ($\approx 0.95$) and favors the power law on parsimony grounds, noting that the two models diverge only far in the future.

This paper contributes four things not found in the prior work, individually or in combination:

\begin{enumerate}[itemsep=2pt, topsep=4pt]
    \item \textbf{Decomposition of the trend into supply drift and adoption transient.} Perrenod's Weibull and the pure power laws lump all growth into a single curve. The model here, $\ln R(t) = C + gt + A(1 - e^{-\lambda t})$, separates a steady-state drift $g \approx 0.02$/yr (Gold's supply dilution) from a decaying adoption surge. This decomposition has a structural consequence: the long-run annual outperformance converges to $\sim$2\%, rather than growing without bound as a power law implies.

    \item \textbf{Falsifiable sequential testing.} No prior analysis publishes explicit rejection thresholds. The CUSUM scorecard (Section~10) provides autocorrelation-corrected boundaries ($c = 8.50$ and $c = 15.9$) calibrated by Monte Carlo simulation, with live tracking at \url{https://silmonbiggs.github.io/BTCvGold/}. The falsification criteria were stated before the out-of-sample period began.

    \item \textbf{Quantitative boom-bust decomposition.} Perrenod observes a ``roughly four-year quasi-periodicity'' from bubble peak spacing and classifies residuals into laminar and turbulent regimes via GMM/HMM. This paper goes further: autocorrelation analysis of the model residuals yields a measured cycle period of $\sim$40 months with lag-1 persistence $\rho = 0.90$, and these values feed directly into the Monte Carlo calibration of the CUSUM boundaries. The distinction matters: regime \emph{labels} describe what happened; autocorrelation \emph{parameters} predict how the next deviation will accumulate.

    \item \textbf{Post-2023 volatility regime change with geopolitical analysis.} Perrenod notes that ``overall volatility has decreased with time'' and that later residual distributions are ``less broad,'' but does not identify a transition date or test for a structural break. This paper pinpoints January 2023, applies a Levene test ($F(1,131) = 23.18$, $p < 0.0001$), measures a 58\% reduction in residual scatter ($\sigma = 0.478 \to 0.258$), documents a rolling decline of $-0.033$/yr ($R^2 = 0.67$, $p < 0.0001$), and argues for a specific causal sequence: the February 2022 freeze of Russian central-bank reserves, the subsequent surge in central-bank Gold purchases ($>$1{,}000 tonnes/yr), the January 2024 spot Bitcoin ETF approvals, and the March 2025 executive order establishing a Strategic Bitcoin Reserve (Section~8).
\end{enumerate}
...
\begin{itemize}
    ...
    \item Lost or abandoned Bitcoin wallets permanently remove coins from circulation, making Bitcoin's effective supply shrink over time. This negative dilution, set against Gold's positive $\sim$2\% dilution, could push the long-run growth differential $g$ well above the 0.02/yr assumed here.
...
\begin{itemize}
    ...
    \item Live model scorecard with daily updates (\url{https://silmonbiggs.github.io/BTCvGold/})
\end{itemize}
...
This paper is academic research, not investment advice. The author is not a financial advisor.
